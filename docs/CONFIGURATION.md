...
- Изменения instance config и списка Telegram-ботов требуют перезапуска демона. Установка, обновление, включение, отключение и удаление pipelines в `pipelines_dir` подхватываются без перезапуска при следующей успешной загрузке каталога.
